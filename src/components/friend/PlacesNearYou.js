  ...
  const searchNearbyPlaces = useCallback(async (mapInstance, location) => {
    ...
    try {
      ...
      if (!response.data.success) {
        throw new Error(
          response.data.message || "Failed to fetch nearby places",
        );
      }
      ...
      places.forEach((place) => {
        ...
        const placeAddress =
          place.address ||
          place.display_name ||
          [place.street, place.city, place.country].filter(Boolean).join(", ");
        const placeCategory = place.category || place.type || "";

        const marker = L.circleMarker([placeLat, placeLng], {
          radius: 7,
          color: "#a16207",
          fillColor: "#facc15",
          fillOpacity: 0.95,
        ...
        const shortAddress =
          typeof placeAddress === "string" && placeAddress.length > 64
            ? `${placeAddress.slice(0, 61)}...`
            : placeAddress;

        marker.bindTooltip(
          `<div><strong>${escapeHtml(placeName)}</strong>${
            shortAddress
              ? `<div style="font-size:11px;opacity:0.9">${escapeHtml(shortAddress)}</div>`
              : ""
          }</div>`,
          {
            permanent: true,
            direction: "top",
            offset: [0, -10],
            className: "places-map-place-label",
            opacity: 0.95,
          },
        );
        ...
        marker.bindPopup(`
                    <div style="padding:8px;max-width:220px;color:#111">
                        <strong style="font-size:14px">${escapeHtml(placeName)}</strong>
                        ${placeCategory ? `<p style="margin:6px 0 0;color:#555;font-size:12px;text-transform:capitalize">${escapeHtml(String(placeCategory).replace(/_/g, " "))}</p>` : ""}
                        <p style="margin:6px 0 0;color:#555;font-size:12px">${escapeHtml(placeAddress || "Address not available")}</p>
                    </div>
                `);
    ...
    async (mapInstance, location, profiles) => {
      ...
      for (const person of profiles) {
        ...
        if (person.profilePic) {
          const circularImage = await createCircularImage(
            person.profilePic,
            40,
          );
          if (circularImage) {
        ...
        marker.bindPopup(`
                <div style="padding:8px;min-width:180px;max-width:240px;color:#111">
                    ...
                    <p style="margin:0;color:#555;font-size:12px">${Number(
                      distance,
                    ).toFixed(1)} km away</p>
                    <button
                        type="button"
        ...
        marker.on("popupopen", () => {
          ...
          const trigger = popupRoot?.querySelector(".js-map-view-profile");
          if (!trigger) return;
    ...
    async (event) => {
      ...
      try {
        ...
        if (mapInstanceRef.current) {
          ...
          searchNearbyPlaces(mapInstanceRef.current, nextLocation);
        }
      } catch (err) {
        console.error("Error searching location:", err);
        setError("Could not search that location right now. Please try again.");
      } finally {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (geoError) => {
        ...
        setError(
          (prev) =>
            prev ||
            "Could not get your location. Showing a default map center.",
        );
      },
  ...
  return (
    <Fragment>
      <div className="places-near-you">
        ...
        <form className="places-map-search" onSubmit={handleLocationSearch}>
          <input
            type="text"
            value={searchQuery}
            onChange={(event) => setSearchQuery(event.target.value)}
            placeholder="Search a location (e.g. New York, Times Square)"
            aria-label="Search location"
            className="places-map-search__input"
          />
          <button
            type="submit"
            className="places-map-search__button"
