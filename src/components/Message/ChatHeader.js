...
import { emotionEmojiMap } from "../../utils/emotionDetection";
...
const ChatHeader = ({
  ...
  const handleMicrophoneClick = useCallback(async () => {
    ...
    if (audioTrack) {
      ...
      console.log(
        "Audio track enabled state after toggle:",
        audioTrack.enabled,
      );
    } else {
  ...
  const handleCameraToggle = useCallback(async () => {
    ...
    if (videoTrack) {
      ...
      console.log(
        "Video track enabled state after toggle:",
        videoTrack.enabled,
      );
    } else {
    ...
    async (channelName) => {
      try {
        ...
        setTimeout(() => {
          ...
          console.log(
            "ChatHeader - Immediate check - Remote users in channel:",
            remoteUsers.length,
          );
          remoteUsers.forEach((user) => {
            console.log("ChatHeader - Remote user details:", {
              uid: user.uid,
        ...
        if (!hasBoundClientEvents.current) {
          ...
          client.on("user-published", async (user, mediaType) => {
            console.log(
              "ChatHeader - Remote user published:",
              user.uid,
              mediaType,
            );
            try {
              ...
              console.log(
                "ChatHeader - Successfully subscribed to",
                user.uid,
                mediaType,
              );
              ...
              if (mediaType === "video") {
                if (userVideo.current && user.videoTrack) {
                  ...
                  console.log(
                    "ChatHeader - Playing remote video from user:",
                    user.uid,
                  );
                } else {
                  console.warn(
                    "ChatHeader - Cannot play remote video - missing userVideo ref or videoTrack",
                  );
                }
              }
              ...
              if (mediaType === "audio") {
                if (user.audioTrack) {
                  ...
                  console.log(
                    "ChatHeader - Playing remote audio from user:",
                    user.uid,
                  );
                } else {
                  console.warn(
                    "ChatHeader - Cannot play remote audio - missing audioTrack",
                  );
                }
              }
            } catch (error) {
              console.error(
                "ChatHeader - Error subscribing to user:",
                user.uid,
                mediaType,
                error,
              );
          ...
          client.on("user-left", async (user) => {
            console.log(
              "ChatHeader - Remote user left the channel:",
              user?.uid,
            );
            try {
              ...
              console.warn(
                "ChatHeader - Cleanup after remote user-left failed:",
                e,
              );
            }
          });
        ...
        setTimeout(async () => {
          try {
            const remoteUsers = client.remoteUsers;
            console.log(
              "ChatHeader - Checking for existing remote users:",
              remoteUsers.length,
            );

            for (const user of remoteUsers) {
              console.log(
                "ChatHeader - Found existing remote user:",
                user.uid,
                "hasVideo:",
                user.hasVideo,
                "hasAudio:",
              ...
              if (user.hasVideo && !user.videoTrack) {
                console.log(
                  "ChatHeader - Subscribing to existing user video:",
                  user.uid,
                );
                ...
                if (userVideo.current && user.videoTrack) {
                  user.videoTrack.play(userVideo.current);
                  console.log(
                    "ChatHeader - Playing existing remote user video",
                  );
                }
              } else if (
                user.hasVideo &&
                ...
                user.videoTrack.play(userVideo.current);
                console.log(
                  "ChatHeader - Playing already subscribed remote video",
                );
              }
              ...
              if (user.hasAudio && !user.audioTrack) {
                console.log(
                  "ChatHeader - Subscribing to existing user audio:",
                  user.uid,
                );
                ...
                if (user.audioTrack) {
                  user.audioTrack.play();
                  console.log(
                    "ChatHeader - Playing existing remote user audio",
                  );
                }
                ...
                user.audioTrack.play();
                console.log(
                  "ChatHeader - Playing already subscribed remote audio",
                );
              }
            }
          } catch (error) {
            console.error(
              "ChatHeader - Error checking for existing remote users:",
              error,
            );
          }
        ...
        remoteUserCheckInterval.current = setInterval(async () => {
          ...
          try {
            ...
            if (remoteUsers.length > 0) {
              console.log(
                `ChatHeader - Periodic check ${checkCount}: Found ${remoteUsers.length} remote users`,
              );
              ...
              for (const user of remoteUsers) {
                ...
                if (user.hasVideo && user.videoTrack && userVideo.current) {
                  ...
                  if (
                    ...
                    console.log(
                      `ChatHeader - Periodic check ${checkCount}: Re-attempting to play remote video for user ${user.uid}`,
                    );
                    userVideo.current.innerHTML = "";
                    user.videoTrack.play(userVideo.current);
                  }
            ...
            console.error(
              `ChatHeader - Error in periodic check ${checkCount}:`,
              error,
            );
          }
  ...
  const handleLeaveCall = useCallback(async () => {
    console.log(
      "ChatHeader: handleLeaveCall - emitting to server and cleaning up",
    );
  ...
  const startVideo = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        "[ChatHeader] Failed to start hidden camera for emotion",
        err,
      );
  ...
  const initializeEmotionServerSocket = useCallback(() => {
    ...
    try {
      ...
      emotionServerSocketRef.current.on("connect", () => {
        console.log(
          "[ChatHeader] ✅ Connected to Python emotion detection server",
        );
      ...
      emotionServerSocketRef.current.on("disconnect", () => {
        console.warn(
          "[ChatHeader] ⚠️ Disconnected from Python emotion detection server",
        );
        isConnectingRef.current = false;
      });
      ...
      console.error(
        "[ChatHeader] Error initializing emotion server socket:",
        error,
      );
      isConnectingRef.current = false;
    }
   ...
   */
  const handleEmotionServerResponse = useCallback(
    (data) => {
      serverRequestInFlightRef.current = false;
      ...
      if (!hasEmotionData) {
        ...
        if (data?.error || data?.message) {
          console.log(
            `[ChatHeader] No face detected: ${data.error || data.message}`,
          );
        }
        return;
      }
      ...
      console.log(
        `[ChatHeader] 🎭 Emotion Detected: ${emoji} ${label} | Category: ${finalEmotion} | Confidence: ${(confidence * 100).toFixed(1)}% | Expression: ${dominantExpression || "none"}`,
      );
      ...
      if (label !== lastMajorityLabelRef.current) {
        ...
        if (profileId && currentFriendId) {
          try {
            ...
            console.log(
              `[ChatHeader] ⚡ FAST Emotion Change Detected: ${previousLabel || "none"} → ${emoji} ${label} | Emitting immediately`,
            );
            ...
            console.error(
              "[ChatHeader] ❌ Error emitting emotion_change:",
              err,
            );
          }
        }
      ...
      while (
        labelHistoryRef.current.length &&
        labelHistoryRef.current[0].t < cutoff
      ) {
      ...
      if (majorityLabel && majorityLabel === label) {
        ...
        const confidenceApprox = Math.max(
          0,
          Math.min(1, majorityCount / windowSize),
        );
        ...
        if (majorityLabel === lastMajorityLabelRef.current) {
          ...
          console.log(
            `[ChatHeader] ✅ Majority confirmed: ${majorityEmoji} ${majorityLabel} | Window Confidence: ${(confidenceApprox * 100).toFixed(1)}% | Window Size: ${windowSize}`,
          );
        }
      }
    },
  ...
  const detectEmotionFromServer = useCallback(
    async (base64Image) => {
      ...
      try {
        ...
        console.warn(
          `[ChatHeader] Error sending frame to Python server (req ${reqId}, ${elapsed}ms):`,
          error.message,
        );
        serverRequestInFlightRef.current = false;
      }
      ...
      if (data && data.emotion) {
        ...
        if (data.expression) {
          if (data.expression !== "none") {
            ...
            setExpression(null);
            console.log(
              '[ChatHeader] ℹ️ Expression is "none", clearing display',
            );
          }
        } else {
          setExpression(null);
        ...
        console.log(
          "[ChatHeader] 📊 Updated state - emotion:",
          data.emotion,
          "expression:",
        ...
        <div className={`chat-header-user ${"skleton-card"}`}>
          ...
          {isLoaded === true ? (
            <div className="chat-header-user-info">
              <h4 className="chat-header-username" title={displayName}>
                {displayName}
              </h4>
              {isMobile ? (
                emotion ? (
      ...
      <div className="chat-header-modals">
        ...
        >
          ...
          >
            ...
            <div
              className={`video-call-container ${isMobile ? "mobile" : ""}`}
              style={{
                width: "100%",
                height: "400px",
                position: "relative",
            ...
            >
              <button
                onClick={handleLeaveCall.bind(this)}
                ref={callEndBtn}
              ...
              {callAccepted && (
                <>
                  <button
                    onClick={handleMicrophoneClick.bind(this)}
