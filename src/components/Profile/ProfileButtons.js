...
const ProfileButtons = (props) => {
    ...
    const [storyBgColors, setStoryBgColors] = useState(false);
    ...
    const profileName = profileData.user && profileData.user.firstName + ' ' + profileData.user.surname
    ...
    let clickUnFrndBtn = async (e) => {
        ...
        try {
            let res = await api.post('/friend/removeFriend', { profile: profileData._id })
                                ...
                                <div className="modal-body">
                                    <div className="cp-modal-container">
                                        ...
                                        <form className="cpm-form" onSubmit={preventDefault}>
                                            ...
                                            <button 
                                                className={`button ${isSubmittingStory ? 'disabled' : ''}`} 
                                                onClick={handleStorySubmit.bind(this)} 
                                                type="submit"
                                                disabled={isSubmittingStory}
                                                style={{ opacity: isSubmittingStory ? 0.6 : 1, cursor: isSubmittingStory ? 'not-allowed' : 'pointer' }}
                                            > 
                                                { isStoryUploading ? "Uploading..." : isSubmittingStory ? "Submitting..." : "Add to your story" }
                                            </button>
                            ...
                            : isReqSent ?
                                ...
                                <div className="profile-buttons">
                                    ...
                                    >
                                        <i className="fas fa-user-check"></i>
                                        <span>
                                            {isCancelingReq ? 'Canceling...' : 'Cancel Request'}
                                        </span>
                                    </div>
                                    <div onClick={clickMessageBtn} className="highligh-btn button message-button">
                                        <i className="fas fa-comment-dots"></i>
                                ...
                                <div className="profile-buttons">
                                    ...
                                    >
                                        <i className="fas fa-user-check"></i>
                                        <span>
                                            {isConfirmingReq ? 'Confirming...' : 'Confirm Request'}
                                        </span>
                                    </div>
                                    <div onClick={clickMessageBtn} className="normal-btn button message-button">
                                        <i className="fas fa-comment-dots"></i>
