/**
 * Gemini API integration for food suggestions and calorie analysis
 */

const GEMINI_API_KEY = process.env.REACT_APP_GEMINI_API_KEY;
const GEMINI_API_URL = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent';

export const getSuggestedMeals = async (params) => {
    if (!GEMINI_API_KEY) {
    ...
    const prompt = `You are a nutrition expert AI assistant. Based on the following information, suggest 3 healthy meal options that are appropriate for the user's goals.

User's Calorie Information:
...
Please provide 3 meal suggestions in the following JSON format (respond with ONLY valid JSON, no additional text):
{
  "suggestions": [
    {
      "name": "Meal name",
...
Make sure each meal fits within the remaining calories and is appropriate for the meal type and health goal. Prioritize meals that are nutritious and balanced.`;
    ...
    try {
        ...
        if (!jsonMatch) {
            throw new Error('Could not parse meal suggestions');
        }
...
export const getCalorieAnalysis = async (params) => {
    if (!GEMINI_API_KEY) {
    ...
    const prompt = `You are a nutrition and fitness expert. Analyze the user's current calorie intake and provide personalized recommendations.
...
- Meals Logged: ${mealsLogged.length}
- Meals: ${mealsLogged.map(m => `${m.name} (${m.calories}kcal)`).join(', ')}

Provide analysis in the following JSON format (respond with ONLY valid JSON):
{
  "analysis": {
    "status": "on-track|under-calories|over-calories",
    "remainingCalories": ${targetCalories - consumedCalories},
    "summary": "Brief assessment of current intake",
    "calorieBreakdown": {
      "target": ${targetCalories},
      "consumed": ${consumedCalories},
      "remaining": ${targetCalories - consumedCalories},
      "percentageConsumed": ${Math.round((consumedCalories / targetCalories) * 100)}
    }
  },
  "recommendations": [
    "recommendation 1",
  ...
  "nextMealSuggestion": {
    ...
    try {
        ...
        console.error('Error getting calorie analysis:', error);
        throw error;
    }
};
