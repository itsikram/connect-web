...
export const AuthProvider = ({ children }) => {
  ...
  const refreshAuthToken = useCallback(async () => {
    try {
      ...
      const response = await axios.post(
        `${getServerAddress()}/api/auth/refresh`,
        {
          refreshToken,
        },
      );
  ...
  const handleLogin = useCallback(async (credentials) => {
    ...
    try {
      const response = await axios.post(
        `${getServerAddress()}/api/auth/login`,
        credentials,
      );
      ...
      if (response.status === 202 && response.data.accessToken) {
        ...
        if (stored) {
          ...
          console.log(
            "✅ Parsed token matches:",
            verification?.accessToken === userData.accessToken,
          );
        } else {
  ...
  const handleGoogleLogin = useCallback(async (googleData) => {
    ...
    try {
      const response = await axios.post(
        `${getServerAddress()}/api/auth/google-signin`,
        googleData,
      );
  ...
  const handleSignup = useCallback(async (signupData) => {
    ...
    try {
      const response = await axios.post(
        `${getServerAddress()}/api/auth/signup`,
        signupData,
      );
  ...
  const handleForgotPassword = useCallback(async (email) => {
    ...
    try {
      const response = await axios.post(
        `${getServerAddress()}/api/auth/forgot-password`,
        {
          email: String(email || "")
            .trim()
            .toLowerCase(),
  ...
  const handleResetPassword = useCallback(
    async (token, password, confirmPassword) => {
      ...
      try {
        const response = await axios.post(
          `${getServerAddress()}/api/auth/reset-password/${token}`,
  ...
  const handleLogout = useCallback(() => {
    ...
    try {
      ...
      if (profileId) {
        ...
        webNotificationService
          .cleanup(profileId, api, { removePush: true })
          .catch(() => {});
      }
    } catch (error) {
    ...
    console.log(
      "✅ User logged out successfully - All data cleared from Redux store",
    );
  }, [dispatch, user]);
